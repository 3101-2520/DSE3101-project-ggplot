...
def render(show_50=False, show_80=False):
    ...
    if len(ses) == len(preds):
            ...
            if show_80:
                fig.add_trace(go.Scatter(
                    ...
                    error_y=dict(
                        ...
                        color='rgba(0, 229, 255, 0.25)', thickness=8, width=0 
                    ),
            ...
            if show_50:
                fig.add_trace(go.Scatter(
                    ...
                    error_y=dict(
                        ...
                        color='rgba(0, 229, 255, 0.35)', thickness=8, width=0 
                    ),
                    hoverinfo='skip', showlegend=False 
                ))

        for i in range(len(preds)):
            htext = f"Prediction: <b>{preds[i]:.2f}%</b>"
        ...
        hover_data = [f"Prediction: <b>{p:.2f}%</b>" for p in preds]

    # --- 1. THE CONNECTING LINE (No dots, No Hover) ---
    # Draws the solid teal line connecting all points, but lets the mouse pass through it
    fig.add_trace(go.Scatter(
        x=months,
        y=preds,
        mode="lines",
    ...
    # --- 2. THE HISTORICAL TEAL DOTS (Everything except the last point) ---
    # Only draws teal markers on the old months, never overlapping the Emerald dot
    if len(preds) > 1:
        fig.add_trace(go.Scatter(
            x=months[:-1],
            y=preds[:-1],
            mode="markers+text",
            text=[f"{p:.2f}%" for p in preds[:-1]],
